    ...
    return (
        <div className='main'>
            ...
            <div className="productlst">
                ...
                {
                    alldata.filter((item) => item.price <= props.pricemax && item.price > props.pricemin).slice(0, 5) // isme starting k 5 element store ho jaayege
                        .map((item, index) => (
                            <Topphone key={index} id={item.id} name={item.model} price={item.price} img={item.img} />
                        ))}
                <div className='moree'>
